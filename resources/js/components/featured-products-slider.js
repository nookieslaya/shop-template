...
function getCardOffsets(track) {
  const cards = Array.from(track.querySelectorAll('.featured-products__item'));
  return cards.map((card) => card.offsetLeft);
}

export function initFeaturedProductsSlider() {
  const sections = document.querySelectorAll('.featured-products[data-slider="true"]');

  sections.forEach((section) => {
    const viewport = section.querySelector('.featured-products__viewport');
    const track = section.querySelector('.featured-products__track');
    const prevButton = section.querySelector('.featured-products__button--prev');
    const nextButton = section.querySelector('.featured-products__button--next');
